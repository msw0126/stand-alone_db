...
//		console.log($("#learn-main").attr('data').substring(0,9))
...
//		}
		//查看组件是否load
		load()
		var line_type = $("#explore-main").attr('souid').substring(0, 6);
		//获取数据渲染
		function appendUl(fields) {
			for(var i = 0; i < fields.length; i++) {
				if(fields[i].selected==true && fields[i].field_type=='factor'){
					$('#c_data_explore_id_list').append('<li>' + fields[i].field + '</li>');
					$('#c_data_explore_target_list').append('<li>' + fields[i].field + '</li>');
				}
			}
		}
		if(line_type === 'RobotX') {
			...
			$.ajax({
				...
				data: {
					project_id: proid,
					component_id: $("#explore-main").attr('souid'),
				},
	...
	$('.right-heads>ul>li').click(function() {
		...
		if(index_lea == 0) {
			...
			$(".c_save_explore_algorithm").hide();
		} else {
			$('.right-content>ul').eq(0).hide();
			$('.right-content>ul').eq(1).show();
			$(".c_save_explore_algorithm").show();
			$("#algorithm_list_explore").html('');
			if(loadStatus === true) {
				load()
				...
				$.ajax({
					type: 'post',
					url: server + '/atom_explore/load_params?' + timeStamp,
					success: function(data) {
						function success() {
	...
	function load(){
			$.ajax({
				type: 'post',
				url: server + '/atom_explore/load?' + timeStamp,
				async: false,
				data: {
					project_id: proid,
					input_comp_id: $("#explore-main").attr('souid'),
					atom_explore_id: $("#explore-main").attr('data')
				},
				success: function(data) {
					function success() {
						...
						if(load_data === 'changed' || load_data === 'null' || load_data === null) {
							...
							loadStatus = true;
							$("#c_data_explore_id").val(load_data.id);
							$("#c_data_explore_id").css('color', "#858585");
							$("#c_data_explore_target").css('color', "#858585");
							$("#c_data_explore_target").val(load_data.target);
	...
	//是否离散化
	$("#algorithm_list_explore").on('click', '.discre>i', function() {
	...
	$('.c_nav_list').click(function() {
		//隐藏全部下拉，避免UI重复
		$('#c_data_explore_id_list').hide();
		$('#c_data_explore_target_list').hide();
		$(this).next().show();
	});
	//下拉选择加载
	$(document).on('click', '#explore-main>.right-content>ul>li>ul>li', function() {
	...
	//保存高级参数
	var params = [];
	$(".c_save_explore_algorithm").click(function() {
		$(".c_save_learn span").remove();
		params = [];
		if(algorithmStatus !== false) {
			var algorithmList = $("#algorithm_list_explore>li");
			...
			if($("#c_data_explore_id").val() === '点击选择数据' || $("#c_data_explore_target").val() === '点击选择数据') {
				alert('请选择变量')
				if($("#c_data_explore_id").val() === '点击选择数据') {
					$("#c_data_explore_id").parent().siblings('.error_learn').show()
				}
				if($("#c_data_explore_target").val() === '点击选择数据') {
					$("#c_data_explore_target").parent().siblings('.error_learn').show()
				}
			} else {
				$.ajax({
					type: 'post',
					url: server + '/atom_explore/saveInfo?' + timeStamp,
					data: {
						project_id: proid,
						atom_explore_id: $("#explore-main").attr('data'),
						input_comp_id: $("#explore-main").attr('souid'),
						id: $("#c_data_explore_id").val(),
						target: $("#c_data_explore_target").val(),
						params: params,
					},
					success: function(data) {
						function success() {
	...
	$('.c_save_learn').click(function() {
		if($("#c_data_explore_id").val() === '点击选择数据' || $("#c_data_explore_target").val() === '点击选择数据') {
			...
			$.ajax({
				type: 'post',
				url: server + '/atom_explore/saveInfoDefualt?' + timeStamp,
				data: {
					project_id: proid,
					atom_explore_id: $("#explore-main").attr('data'),
					input_comp_id: $("#explore-main").attr('souid'),
	...
	function success_list(algorithmParams, statusLoad) {
		...
		for(var i = 0; i < algorithmParams.length; i++) {
			...
			if(algorithmParams[i].type === "bool") {
				var class_sample = '';
				if(statusLoad == 'true'){
					if(algorithmParams[i].value.toLowerCase()=='true') {
		...
		$("#algorithm_list_explore>li>div>span").each(function() {
		...
		//分类点击事件
		$("#algorithm_list_explore").on('click','.c_nav_list_metric',function(){
		...
		$("#algorithm_list_explore").on('click','#metric_list>li',function(){
			var selectEm = $(this).parent().siblings('.c_nav_list_metric').find('span')
			selectEm.text($(this).text())
		});
		//校验高级参数
		var algorithmList = $("#algorithm_list_explore>li").not('.discre');
